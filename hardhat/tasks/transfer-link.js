...
task("transfer-link", "Transfer LINK tokens to a recipient")
    ...
    .setAction(async (taskArgs) => {
        ...
        console.log(`LINK balance of sender ${signer.address} is ${ethers.formatEther(balance)}`);
        ...
        if (balance >= amountBN) {
            ...
            await result.wait();
            console.log(
                `${ethers.formatEther(amountBN)} LINK where sent from sender ${
                    signer.address
                } to ${recipientAddress}.Transaction Hash: ${result.hash}`
            );
        } else {
            console.log(
                `Sender doesn't have enough LINK. Current balance is ${ethers.formatEther(
                    balance
                )} LINK and transfer amount is ${ethers.formatEther(amount)} LINK`
            );
        }
    });
